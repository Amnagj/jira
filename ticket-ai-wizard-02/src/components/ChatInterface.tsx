...
import { Bot, Search , Clock} from "lucide-react";
...
export const ChatInterface = ({ initialMessage, ticketIds }: ChatInterfaceProps) => {
  ...
  const MessageComponent = ({ message }: { message: Message }) => {
    // Fonction pour extraire le temps de recherche du message s'il existe
    const extractSearchTime = (content: string): string | null => {
      const timeRegex = /\*Temps de recherche: ([\d.]+)s\*/;
      const match = content.match(timeRegex);
      return match ? match[1] : null;
    };

    // Extraction du temps de recherche si présent dans le message
    const searchTime = extractSearchTime(message.content);
    
    // Nettoyer le contenu pour afficher sans la ligne de temps (puisqu'on va l'afficher différemment)
    const cleanedContent = message.content.replace(/\*Temps de recherche: [\d.]+s\*/, '');
